...
|---|---|---|---|---|---|
| **Attendance is not a readable number** — check-in writes `checkedInAt` on one ticket at a time and nothing aggregates it. "How many people have walked into tonight's show" is unanswerable through the contract: `ReportSummary` and `ReportByEventRow` count `ticketsSold`, `InventoryRow` counts capacity/sold/reserved/available, and none of them counts anybody through the door. The only path today is exporting attendees and counting rows client-side, and that export is an untyped `{type: object}` (#689) | `checkedIn` on `InventoryRow` and `ReportByEventRow` (additive) **+** `GET /reports/attendance?event=&eventDate=` for the live door count (checked in, sold, rate, last check-in) | B2B | cli, mcp | — | identified |
