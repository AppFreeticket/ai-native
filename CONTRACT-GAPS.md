...
|---|---|---|---|---|---|
| **Estados de evento y fecha no se pueden escribir** ⚠️ — el enum tiene `CANCELLED`/`SOLD_OUT`/`COMPLETED` y la única transición expuesta es `DRAFT→PUBLISHED`. Cancelar un show obliga a `DELETE`, que borra el historial de ventas. Una fecha agregada después de publicar no se puede publicar | `POST /events/{id}/cancel` + escritura de `status` en `EventDateUpdate` | B2B | cli, mcp | [#683](https://github.com/AppFreeticket/free-admin/issues/683) | requested |
| **Dar de baja staff es inejecutable** ⚠️ — se puede invitar y cambiar rol; `/staff/{id}` no existe. Bajar a `VIEWER` no revoca: la persona sigue viendo ventas y compradores | `DELETE /staff/{id}` + `GET /staff/{id}` | B2B | cli, mcp | [#684](https://github.com/AppFreeticket/free-admin/issues/684) | requested |
| `Sale` no dice por qué falló — `ABANDONED` con `confirmedAt: null` es indistinguible de una orden expirada. Diagnosticar exige salir de la API y mirar logs de pasarela en producción | `failureReason`/`declineCode` en `Sale`, o `GET /sales/{id}/attempts` | B2B | cli, mcp | [#685](https://github.com/AppFreeticket/free-admin/issues/685) | requested |
| El ciclo de liquidación no cierra headless — hay dos sub-recursos de archivo colgando de un id que no se puede leer, y ningún `POST` bajo `settlement` | `GET /settlements/{id}` + `POST /settlements/{id}/resend` | B2B | cli, mcp | [#686](https://github.com/AppFreeticket/free-admin/issues/686) | requested |
| Webhooks sin edición ni historial — no hay forma de ver si una entrega falló, ni de reintentarla; editar obliga a borrar y recrear, lo que rota el `signingSecret` | `PATCH /webhooks/{id}` + `GET /webhooks/{id}/deliveries` + retry | B2B | cli, mcp | [#687](https://github.com/AppFreeticket/free-admin/issues/687) | requested |
| Reasignar un ticket vendido a otra fecha — un aplazamiento obliga a reembolsar y revender, perdiendo cargo de pasarela, asistencia y silla | `PATCH /tickets/{ticketCode}` con `eventDateId`/`ticketTypeId` | B2B | cli, mcp | [#688](https://github.com/AppFreeticket/free-admin/issues/688) | requested |
| Exports sin schema ni dato fiscal — `/reports/exports/*` responde `{type: object}` sin propiedades, y `Sale` no tiene documento de identidad para facturar | tipar la respuesta de los tres exports + `buyerDocumentType`/`buyerDocumentNumber` | B2B | cli, mcp | [#689](https://github.com/AppFreeticket/free-admin/issues/689) | requested |
| Conciliación sin filtro por evento — hay que traer todo el workspace y cruzar a mano por `sale_reference`; la fila tampoco dice a qué evento pertenece | `event`/`eventDate` en `/reports/reconciliation` + `eventId`/`eventName` en `ReconciliationRow` | B2B | cli, mcp | [#690](https://github.com/AppFreeticket/free-admin/issues/690) | requested |
| Superadmin: el plan se asigna a ciegas y `deletedAt` es inalcanzable — `POST /workspaces/{id}/plan` cancela Stripe sin que se pueda leer el plan actual; el schema declara una baja que ningún endpoint ejecuta | `plan` en `AdminWorkspace` + `DELETE /workspaces/{id}` | Superadmin | cli, mcp | [#691](https://github.com/AppFreeticket/free-admin/issues/691) | requested |
| **Los schemas de escritura no cubren los de lectura** — `saleStartsAt`/`saleEndsAt`, `EventDate.status`, `streamType` y las claves de Seats.io se leen y no se pueden escribir. Programar cuándo abre la venta es imposible headless | esos campos en los `*Create`/`*Update`, o `readOnly: true` si es deliberado | B2B | cli, mcp | [#692](https://github.com/AppFreeticket/free-admin/issues/692) | requested |
| El checkout público no acepta sillas — `SaleCreate` tiene `items[].seats` y `CreateOrderRequest` no: la compra por agente está cerrada para todo evento numerado | `items[].seats` en `CreateOrderRequest` + sillas en `/availability` | B2C (`/api/public`) | mcp | [#693](https://github.com/AppFreeticket/free-admin/issues/693) | requested |
| Preventa members-only sin contraparte — `benefitPresale` es un booleano del plan y ningún evento puede declararse restringido a miembros. Requiere decisión de producto antes que endpoint | confirmar si el feature existe en el panel; después contractualizarlo | B2B | skills | [#694](https://github.com/AppFreeticket/free-admin/issues/694) | requested |
| Un organizador no puede subir de plan solo — el comprador tiene `POST /customer/subscriptions` con link de Stripe; el organizador depende de venta asistida de superadmin. Puede ser deliberado | `POST /billing/checkout` en v1, o cerrar como decisión comercial | B2B | cli, mcp | [#695](https://github.com/AppFreeticket/free-admin/issues/695) | requested |
| **`buyerTotal` miente en eventos numerados** ⚠️ — `/api/v1` no suma el recargo de silletería ($3.000 por silla) que el checkout sí cobra. Un agente que cotiza con el contrato le dice al comprador un precio menor al que va a pagar. Es plata, y el número equivocado lo entregamos nosotros | `buyerTotal` del `TicketTypeDTO` tiene que incluir `SEATING_SURCHARGE_PER_SEAT` cuando la función es numerada, o el contrato tiene que exponer el recargo aparte para que el cliente lo sume | B2B | cli, mcp | [#529](https://github.com/AppFreeticket/free-admin/issues/529) | requested |
| Transferencia de entrada a otra persona por API — escindido de #355, era su único punto pendiente. Un comprador que quiere pasarle su entrada a alguien depende del panel | endpoint de transferencia sobre el ticket (reasigna titular y reemite el QR) | B2B (`/customer`) | mcp | [#507](https://github.com/AppFreeticket/free-admin/issues/507) | requested |
...
abajo. Lo que el contrato **no expone**, o expone mal, son las **veinte filas
abiertas** de arriba, y no las ve ningún barrido automático: salen de auditar el
producto desde la posición de un agente. Las más urgentes: #529 (`buyerTotal`
miente en eventos numerados — es plata), #673 (no hay onboarding), #683 (cancelar
un evento obliga a borrarlo) y #684 (no se puede dar de baja a un empleado). El barrido detecta endpoints
