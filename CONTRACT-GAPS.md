...
| Descubrimiento público B2C (catálogo para agentes de compradores) | `GET /public/events` (filtros `city`, `date`, `category`, `q`, cursor), `GET /public/events/{slug}`, `GET /public/events/{slug}/availability` (fechas, tipos, precios, stock) — sin auth, cache-friendly, solo publicados | B2C (`/api/public`, nuevo) | cli, mcp | [#189](https://github.com/AppFreeticket/free-admin/issues/189) | requested |
| Compra por agente B2C (checkout vía Mercado Pago, el agente nunca toca el pago) | `POST /public/orders` (buyer email + items, header `Idempotency-Key` obligatorio, rate limit → `order_id` + `checkout_url` MP), `GET /public/orders/{id}` (estado `pending\|paid\|expired\|cancelled` + tickets al pagar) | B2C (`/api/public`, nuevo) | cli, mcp | [#190](https://github.com/AppFreeticket/free-admin/issues/190) | requested |
| Post-venta comprador B2C (reenvío de ticket sin credenciales) | `POST /public/tickets/{code}/resend` (reenvía QR/email al mail del comprador, rate-limited, email enmascarado en la respuesta) | B2C (`/api/public`, nuevo) | cli, mcp | [#191](https://github.com/AppFreeticket/free-admin/issues/191) | requested |

| Authorization server OAuth 2.1 para el MCP remoto — claude.ai exige OAuth para connectors con credenciales; el mcp ya expone el resource-server side (RFC 9728) pero falta el AS que emita tokens mapeados a API key + workspace | `/.well-known/oauth-authorization-server` (metadata RFC 8414), dynamic client registration (RFC 7591), authorize + token con PKCE, y una página de consentimiento (reusar la de device flow `/cli`) en free-admin | B2B | mcp | — | identified |
